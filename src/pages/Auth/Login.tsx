...
const Login = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Check if user email is verified using our custom verification
    ...
    <div className="rounded-2xl border border-border/50 bg-background/70 backdrop-blur-xl p-8 sm:p-10 shadow-[0_20px_60px_-30px_rgba(0,0,0,0.6)]">
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        <div className="space-y-2">
          ...
          <Input id="email" type="email" placeholder="you@example.com" className="h-12 bg-secondary/50 border-border/50 focus:border-primary/50 transition-colors" value={email} onChange={e => setEmail(e.target.value)} required />
        </div>
        ...
        <div className="space-y-2">
          ...
          <div className="relative">
            <Input id="password" type={showPassword ? "text" : "password"} placeholder="Enter your password" className="h-12 bg-secondary/50 border-border/50 focus:border-primary/50 pr-12 transition-colors" value={password} onChange={e => setPassword(e.target.value)} required />
