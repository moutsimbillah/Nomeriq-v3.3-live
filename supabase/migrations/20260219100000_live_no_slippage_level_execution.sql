-- Live mode no-slippage execution:
-- - Auto SL/TP trigger checks still use live quote touch.
-- - Executed close price is the configured level price (SL/TP), not trigger quote.
-- - Limit TP partial P&L is computed from TP level price, not trigger quote.
...
BEGIN
  ...
  LOOP
    ...
    LOOP
      ...
      -- No slippage mode: execute the partial at configured TP level.
      v_applied_count := public.apply_signal_take_profit_update_row(update_row.id, update_row.tp_price);
