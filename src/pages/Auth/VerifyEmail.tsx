...
const VerifyEmail = () => {
  ...
  useEffect(() => {
    if (!email) {
      toast({
        ...
        description: "Please sign up again to verify your email.",
        variant: "destructive",
      });
      navigate("/signup");
    }
  ...
  // Auto-submit when 6 digits entered (with guard to prevent double submission)
  useEffect(() => {
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      // Handle function invocation errors (network, deployment issues)
      if (error) {
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      // Handle rate limiting or other errors from the function
      if (data?.error) {
