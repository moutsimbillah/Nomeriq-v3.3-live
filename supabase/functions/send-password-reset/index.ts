...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const { data: emailSettings } = await supabaseAdmin
      ...
      .select("sender_name, sender_email, reset_subject, reset_body")
      .limit(1)
      .maybeSingle();
    ...
    const subjectTemplate = emailSettings?.reset_subject || "Your {{brand_name}} password reset code";
    const bodyTemplate =
      emailSettings?.reset_body ||
